...
public class MailDialog extends CloseDialog implements MailCallback
{
	...
	public MailDialog (Frame f, String s)
	{
		...
		p.add(From = new FormTextField(Global.getParameter("from", Global
			.resourceString("Your_EMail_Address"))));
		p.add(new MyLabel("To : "));
		p.add(To = new FormTextField(Global.getParameter("to", Global
			.resourceString("Destination_EMail_Address"))));
		...
		p
			.add(Host = new GrayTextField(Global.getParameter("smtp",
				"SMTP host")));
		p.add(new MyLabel(Global.resourceString("This_host___")));
		try
		{
			p
				.add(new MyLabel(InetAddress.getByName("localhost")
					.getHostName()));
		}
		catch (Exception e)
		{
	...
	public void doAction (String o)
	{
		...
		if (Global.resourceString("Send").equals(o))
		{
			...
			SendMail s = new SendMail(Host.getText(), To.getText(), From
				.getText());
